...
public class SoundControlsExample extends SimpleFramework {
	...
	protected void render(Graphics g) {
		super.render(g);
		textPos = Utility.drawString(g, 20, textPos, "", "Clip Gain: " + clip.getGain(), "Clip Pan: " + clip.getPan(),
				"", "Stream Gain: " + dataLine.getGain(), "Stream Pan: " + dataLine.getPan(), "",
				"(1) Start Loop (clip)", "(2) Stop Loop (clip)", "(3) Start Loop (stream)", "(4) Stop Loop (stream)",
				"", "(W) Raise Gain (clip)", "(S) Lower Gain (clip)", "(A) Pan Left (clip)", "(D) Pan Right (clip)", "",
				"(I) Raise Gain (stream)", "(K) Lower Gain (stream)", "(J) Pan Left (stream)",
				"(L) Pan Right (stream)");
	}
